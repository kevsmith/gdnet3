...
void GDNetHost::releaseMutex() { _hostMutex->unlock(); }
...
void GDNetHost::send_messages() {
  while (!_message_queue.is_empty()) {
    ...
    PENetPacket* penet_packet =
        penet_packet_create(r.ptr(), message->get_packet().size(), flags);

    if (penet_packet != NULL) {
      if (message->is_broadcast()) {
        penet_host_broadcast(_host, message->get_channel_id(), penet_packet);
      } else {
        penet_peer_send(&_host->peers[message->get_peer_id()],
                        message->get_channel_id(), penet_packet);
      }
    }
...
Error GDNetHost::bind(Ref<GDNetAddress> addr) {
  ...
  if (addr.is_null()) {
    _host = penet_host_create(NULL, _max_peers, _max_channels,
                              _max_bandwidth_in, _max_bandwidth_out);
  } else {
    ...
    if (host_addr.length() == 0) {
      ...
      if (penet_address_set_host(&penet_addr, host_addr.get_data()) != 0) {
        print_error("Unable to resolve host");
        return FAILED;
      }
    }

    _host = penet_host_create(&penet_addr, _max_peers, _max_channels,
                              _max_bandwidth_in, _max_bandwidth_out);
  }
...
void GDNetHost::broadcast_packet(const PoolByteArray& packet, int channel_id,
                                 int type) {
...
bool GDNetHost::is_event_available() { return (!_event_queue.is_empty()); }

int GDNetHost::get_event_count() { return (_event_queue.size()); }

Ref<GDNetEvent> GDNetHost::get_event() { return (_event_queue.pop()); }

void GDNetHost::_bind_methods() {
  ClassDB::bind_method(D_METHOD("get_peer"), &GDNetHost::get_peer);

  ClassDB::bind_method(D_METHOD("set_event_wait"),
                       &GDNetHost::set_event_wait);  // Deprecated
  ClassDB::bind_method(D_METHOD("set_max_peers"), &GDNetHost::set_max_peers);
  ClassDB::bind_method(D_METHOD("set_max_channels"),
                       &GDNetHost::set_max_channels);
  ClassDB::bind_method(D_METHOD("set_max_bandwidth_in"),
                       &GDNetHost::set_max_bandwidth_in);
  ClassDB::bind_method(D_METHOD("set_max_bandwidth_out"),
                       &GDNetHost::set_max_bandwidth_out);

  ClassDB::bind_method(D_METHOD("bind"), &GDNetHost::bind, DEFVAL(NULL));
  ClassDB::bind_method(D_METHOD("unbind"), &GDNetHost::unbind);
  ClassDB::bind_method(D_METHOD("host_connect"), &GDNetHost::host_connect,
                       DEFVAL(0));
  ClassDB::bind_method(D_METHOD("broadcast_packet"),
                       &GDNetHost::broadcast_packet, DEFVAL(0),
                       DEFVAL(GDNetMessage::UNSEQUENCED));
  ClassDB::bind_method(D_METHOD("broadcast_var"), &GDNetHost::broadcast_var,
                       DEFVAL(0), DEFVAL(GDNetMessage::UNSEQUENCED));
  ClassDB::bind_method(D_METHOD("is_event_available"),
                       &GDNetHost::is_event_available);
  ClassDB::bind_method(D_METHOD("get_event_count"),
                       &GDNetHost::get_event_count);
